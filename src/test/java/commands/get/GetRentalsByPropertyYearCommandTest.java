...
public class GetRentalsByPropertyYearCommandTest {
    ...
    private Map<MimeType, HttpContent> runCommandAndGetViews() throws CommandFailedException, NoSuchDataException {
        ...
        ICommand cmd =new GetRentalsByPropertyYearCommand("/properties/{pid}/rentals/{year}");

        IRequest request = TestUtils.getConsoleRequest("GET", "/properties/1/rentals/2014", "");
    ...
    public void testRunWithValidValues() throws Exception{
        ...
        //Text view should have a rental with idProp = 1, year = 2014, week = 1 and week = 13
        StringWriter sw = new StringWriter();
        views.get(new MimeType("text/plain")).writeTo(sw);
        String result = sw.toString();
        
        assertTrue( result.contains("idProp = 1"));
        assertTrue( result.contains("userName = bia"));
        assertTrue( result.contains("userName = carlos"));
        assertTrue( result.contains("year = 2014"));
        assertTrue( result.contains("week = 3"));
        assertTrue( result.contains("week = 1"));
    }
