...
public class GetPropertiesByTypeCommandTest {
    ...
    private Map<MimeType, HttpContent> runCommandAndGetViews() throws CommandFailedException, NoSuchDataException {
        ICommand cmd = new GetPropertiesByTypeCommand("/properties/type/{type}");

        IRequest request = TestUtils.getConsoleRequest("GET", "/properties/type/apartamento", "");
    ...
    public void testRunWithValidValues() throws Exception{
        ...
        //Text view should have only a property with id = 3 and owner = carlos
        StringWriter sw = new StringWriter();
        views.get(new MimeType("text/plain")).writeTo(sw);
        String result = sw.toString();
        
        assertFalse( result.contains("id = 1"));
        assertFalse( result.contains("id = 2"));
        assertFalse( result.contains("owner = oksana"));
